...
class m140821_114749_add_spo2 extends CDbMigration
{
	...
	private function getTypes()
	{
		...
		$this->event = $this->dbConnection->createCommand()
			->select('e.*')
			->from('event e')
	...
	public function up()
	{
		...
		// Find all patient measurements for this event for this measurement type (sao2)
		$patient_measurements = $this->dbConnection->createCommand()
			...
			->where('mr.event_id = :ei AND pm.measurement_type_id = :mti', array(
				':ei' => $this->event['id'],
	...
	public function down()
	{
		...
		// Find all patient measurements for this event for this measurement type (spo2)
		$patient_measurements = $this->dbConnection->createCommand()
			->select('pm.*')
			->from('measurement_reference mr')
			->join('patient_measurement pm', 'pm.id = mr.patient_measurement_id')
